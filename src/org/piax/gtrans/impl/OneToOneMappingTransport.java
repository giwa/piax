...
public abstract class OneToOneMappingTransport<E extends Endpoint> extends
    ...
    public void onReceive(Channel<E> lowerCh) {
        ...
        synchronized (lowerCh) {
            ...
            NestedMessage nmsg = (NestedMessage) lowerCh.receive();
            if (nmsg == null) {
    ...
    protected void _onReceive(Channel<E> lowerCh, NestedMessage nmsg) {
        /*
         * TODO think!
         * この処理では、下位から受信したメッセージを一回lowerCh.receiveしてから、
         * このTransportの持つChannelのBlockingQueueにputしている。
         * この場合は、下位層のBlockingQueueを共有することが効率的であるが、要件等
         */
        logger.trace("ENTRY:");
        ...
        if (ch == null) {
            ...
            //System.out.println("accepted channel:" + ch + "lower No=" + lowerCh.getChannelNo());
            // listenerにdispatchする
            ChannelListener<E> listener = getChannelListener(nmsg.receiver);
            //System.out.println("_onReceive ch=" + ch);
            if (listener != null) {
