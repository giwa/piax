...
public interface NodeManagerIf extends RPCIf {
    ...
    void setR(DdllKey target, Link sender, int reqNo, Link rNew, Link rCur,
            LinkNum rNewNum, int incr, Object payload) throws RPCException;
    ...
    @RemoteCallable(Type.ONEWAY)
    void setRNak(DdllKey target, Link sender, int reqNo) throws RPCException;
    ...
    @RemoteCallable(Type.ONEWAY)
    void propagateNeighbors(DdllKey key, Set<Link> newset, DdllKey limit)
            throws RPCException;
