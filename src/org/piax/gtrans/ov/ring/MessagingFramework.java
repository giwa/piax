...
public class MessagingFramework {
    ...
    public void ackMsgReceived(int ackId, AckMessage ack) {
        ...
        if (req == null) {
            ...
            logger.info("{}: no corresponding request (maybe ok): ackId={}", h,
                    ackId);
        } else {
    ...
    public void replyMsgReceived(ReplyMessage repl) {
        ...
        if (req2 == null) {
            logger.info("{}: no corresponding request (maybe ok): replyId={}",
                    h, repl.replyId);
        } else {
    ...
    public void dispose(int replyId) {
        msgStore.remove(replyId);
        logger.debug("MessageFramework#dispose: id={}, msgStore={}", replyId,
                msgStore.keySet());
    }
