...
public class Main {
	static public void main(String args[]) throws Exception {
		...
		AgentPeer p = new AgentPeer(selfHostName + ":" + selfPort, 
				selfLocator, seedLocator, new File("./"));
        ...
        stub.setup();
		Thread.sleep(100);;
        System.out.println(p.getTransportManager().getRPCTransport().getBaseTransport().getEndpoint() + " is ready.");
		Thread.sleep(60000);
		p.fin();
		System.out.println(p.getTransportManager().getRPCTransport().getBaseTransport().getEndpoint() + " finished running.");
	}
}
